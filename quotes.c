...
char	*process_single_quotes(char *line, t_position_tracker *pos, t_command **cmd, t_list *mini)
{
	int		quote_pos;
	...
	if (line[quote_pos] == '\'')
	{
		single_quote_str = create_inside_quotes_token(line, pos->i + 1, quote_pos - 1);
...
char	*process_double_quotes(char *line, t_position_tracker *pos, t_command **cmd, t_list *mini)
{
	int		quote_pos;
	...
	if (line[quote_pos] == '"')
	{
		double_quote_str = create_inside_quotes_token(line, pos->i + 1, quote_pos - 1);
		//update mini
...
void	process_in_quotes(char *line, t_position_tracker *p, t_command **cmd, t_list *mini)
{
	char	*temp;
